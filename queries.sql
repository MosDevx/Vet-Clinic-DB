...
-- queries for vets visits and specialations
...
SELECT a.name FROM animals a JOIN visits vi ON a.id = vi.animal_id JOIN vets v ON vi.vet_id = v.id WHERE v.name ='Stephanie Mendez' AND vi.date >= '2020-04-01'AND vi.date <= '2020-08-30';
...
SELECT animal_id, a.name , date_of_birth, escape_attempts, neutered, weight_kg, species_id, v.name AS vet_name, v.age, v.date_of_graduation, vi.date AS date_of_visit FROM visits vi LEFT JOIN animals a ON a.id = vi.animal_id LEFT JOIN vets v on v.id = vi.vet_id ORDER BY date DESC LIMIT 1; 
